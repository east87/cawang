...
function submitContactForm() {
    ...
    $.ajax({
        type: 'POST',
        url:'https://cawang.co.id/contact/postMsg',
        data: data,
    beforeSend: function() {
...
function myFunctionDeal() {
        ...
        $("#form-dealer").validate({
            ...
            messages: {
               ...
               dealer_email: "Isi alamat email yang benar",
                dealer_hp: {
                    required: "Isi Telepon anda",
                    minlength: "Minimum 3 huruf",
                    maxlength: "Maximum 30 huruf"
                },
               dealer_lokasi: {
                    required: "Isi lokasi anda",
                    minlength: "Minimum 5 huruf"
                }
            },
            submitHandler: submitDealForm
        });
        /* validation */
    }
    /* form submit */
function submitDealForm() {
  ...
  //  alert(data);
    $.ajax({
        type: 'POST',
        url: 'https://cawang.co.id/contact/postDeal',
        data: data,
    beforeSend: function() {
     var btnSend = '<span class="input-group-append">' +
            '<i class="fa fa-spinner fa-spin" aria-hidden="true"></i>';
            $("#btn-deal").html(btnSend);
  }
    }).done(function(result) {
        $("#btn-deal").html("&nbsp;SUBMIT");
        if (result == 1) {
            $('#dealer_name').val('');
            $('#dealer_email').val('');
            $('#dealer_hp').val('');
